...
public class CreateRequestsControllerTests {
	...
	public void createFullRequest() throws Exception {
		...
		SmallRequestDTO result = mapper.readValue(response, SmallRequestDTO.class);
		assertThat(result).usingRecursiveComparison()
				.ignoringFields("id", "lastUpdated", "client.id", "closedBy.id",
						"estimation.estimationResponsable.id", "estimation.id",
						"lastUpdatedBy.id", "responsable.id")
				.isEqualTo(expected);
		assertThat(result.getId()).isNotZero();
		assertThat(result.getLastUpdated()).isNotNull();
	}
	...
	public void uploadImage() throws Exception {
		...
		MockMultipartFile fstmp = new MockMultipartFile("images", f.getName(),
				"multipart/form-data", fi1);
		MockMultipartFile fstmp2 = new MockMultipartFile("images", f2.getName(),
				"multipart/form-data", fi2);
		...
		assertThat(updatedSmallReq).usingRecursiveComparison()
				.ignoringFields("photos", "lastUpdated").isEqualTo(smallReq);
		...
		this.mockMvc
				.perform(get(updatedSmallReq.getPhotos().get(0))
						.contentType(MediaType.IMAGE_JPEG)
						.headers(TestUtils.userAuthorizationHeader(mockMvc)))
				.andExpect(status().isOk()).andReturn().getResponse()
				.getContentAsByteArray().equals(firstImage);

		byte[] secondImage = Files.readAllBytes(f2.toPath());

		this.mockMvc
				.perform(get(updatedSmallReq.getPhotos().get(1))
						.contentType(MediaType.IMAGE_JPEG)
						.headers(TestUtils.userAuthorizationHeader(mockMvc)))
				.andExpect(status().isOk()).andReturn().getResponse()
				.getContentAsByteArray().equals(secondImage);
